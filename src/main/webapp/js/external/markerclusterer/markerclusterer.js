...
function Cluster(markerClusterer) {
  ...
  this.redraw_ = function (isForce) {
    ...
    if (zoom_ >= mz || this.getTotalMarkers() === 1) {
      ...
      // show only 1 marker
      if (markers_[0].isAdded) {
          if (markers_[0].marker.isHidden()) {
            markers_[0].marker.show();
          }
        } else {
          map_.addOverlay(markers_[0].marker);
          markers_[0].isAdded = true;
        }
      
      // the cluster marker has been disabled for the BOM portal. we want to show 1 marker instead
      if (clusterMarker_ === null) {
